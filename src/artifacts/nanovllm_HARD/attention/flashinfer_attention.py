...
from src.services.nanovllm_HARD.utils.logging import append_lse_log
from src.services.nanovllm_HARD.engine.sequence import Sequence
...
def store_q_cache_kernel_decode(
    q_cache_ptr, query_slot_mapping_ptr, query_ptr, query_stride, D: tl.constexpr
):
    idx = tl.program_id(0)
    query_slot = tl.load(query_slot_mapping_ptr + idx)
    cache_offsets = query_slot * D + tl.arange(0, D)
...
def store_q_cache(
    ...
    query_window_pos: torch.Tensor = None,
    ...
    if is_prefill:
        ...
        store_q_cache_kernel_prefill[(N,)](
            q_cache, query_slot_mapping, query_window_pos, query, query.stride(0), D
        )
    else:
        store_q_cache_kernel_decode[(N,)](
            q_cache, query_slot_mapping, query, query.stride(0), D
        )
...
def read_q_cache(q_cache: torch.Tensor, query_slot_mapping: torch.Tensor):
    ...
    ](q_cache, query_slot_mapping, query, L, D)
    return query
...
def read_kvcache(
    ...
    key = torch.empty(
        (N, num_kv_heads, head_dim), dtype=k_cache.dtype, device=k_cache.device
    )
    value = torch.empty(
        (N, num_kv_heads, head_dim), dtype=v_cache.dtype, device=v_cache.device
    )

    read_kvcache_kernel[(N,)](
        k_cache, v_cache, slot_mapping, key, key.stride(0), value, value.stride(0), D
    )
...
class Attention(nn.Module, Artifact):
    ...
    def __init__(
        ...
        self.custom_mask_buf = torch.zeros(
            (model_runner.config.max_model_len * max_bs * num_kv_heads // 8,),
            dtype=torch.uint8,
    ...
    def prepare_metadata_for_attn_prefill(self, seqs: list[Sequence]):
        ...
        kv_page_indices = torch.tensor(
            list(
                itertools.chain(
                    *[seq.block_table[: seq.num_cached_blocks] for seq in seqs]
                )
            ),
            device="cuda",
        ).to(torch.int32)
        ...
        self.prefill_metadata = PrefillMetadata(
            use_ragged=True, no_prefix=context.no_prefix
        )
    ...
    def prepare_metadata_for_attn_decode(
        self, qo_indptr, kv_indptr, kv_page_indices, cu_packed_custom_mask
    ):
    ...
    def attn(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, layer_id: int):
        ...
        if context.is_prefill:
            store_q_cache(
                q,
                self.q_cache,
                context.query_slot_mapping,
                context.query_window_pos,
                is_prefill=True,
            )
