...
BatchPrefillWithPagedKVCacheWrapper.partial_update_custom_mask = (
    page_partial_update_mask
)
...
def inplace_rewrite_flattened(
    k_cache, v_cache, list_of_src_indices, list_of_dst_indices
):
    """
    Args:
...
class BenchMark:
    def __init__(self):
        ...
        self.global_block_table = torch.zeros(
            (256, 2048 * 10 * 64), dtype=torch.int32, device="cuda"
        )
        self.rewrite_block_table = torch.zeros(
            (32 * 8 * 36, 2048 * 10), dtype=torch.int32, device="cuda"
        )
    ...
    def LS_generate_layouts(self, num_seqs, seq_length, num_selected):
        ...
        mask_indptr = torch.cumsum(
            torch.tensor(mask_indptr, dtype=torch.int32, device="cuda"), dim=0
        )

        masks_sum = {layer_id: [] for layer_id in range(num_layers)}
        mask_indptr_sum = [0] + [seq_length_sum_layers] * num_seqs
        mask_indptr_sum = torch.cumsum(
            torch.tensor(mask_indptr_sum, dtype=torch.int32, device="cuda"), dim=0
        )
        ...
        for seq_id in range(num_seqs):
            ...
            if self.mode == "LS_rewrite_prefill":
                self.global_block_table[seq_id, :num_selected] = block_table_ids[
                    :num_selected
                ].to(torch.int32)
            if self.mode == "LS_sparse_prefill":
                self.global_block_table[seq_id, :seq_length] = block_table_ids[
                    :seq_length
                ].to(torch.int32)
            if self.mode == "LS_flatten_sum_prefill":
                self.global_block_table[seq_id, :seq_length_sum_layers] = (
                    block_table_ids.to(torch.int32)
                )
            ...
            for layer_id in range(num_layers):
                ...
                cu_mask_sum = torch.zeros(
                    (seq_length_sum_layers,), dtype=torch.bool, device="cuda"
                )
                cu_mask_sum[
                    selected_indices_all_layers[layer_id] : selected_indices_all_layers[
                        layer_id + 1
                    ]
                ] = True
                masks_sum[layer_id].append(cu_mask_sum)

                cu_mask_sparse = torch.zeros(
                    (seq_length,), dtype=torch.bool, device="cuda"
                )
                cu_mask_sparse[selected_indices_sparse] = True
                masks_sparse[layer_id].append(cu_mask_sparse)

                if self.mode == "LS_rewrite_prefill":
                    self.src_page_indptr_list.append(
                        block_table_ids[selected_indices_sparse].to(torch.int32)
                    )
                    self.dst_page_indptr_list.append(
                        self.global_block_table[seq_id, :num_selected].to(torch.int32)
                    )

        if self.mode == "LS_sparse_prefill":
            for layer_id in range(num_layers):
                cu_mask_sparse = (
                    torch.cat(masks_sparse[layer_id], dim=0).contiguous().view(-1)
                )
                cu_packed_mask, _ = segment_packbits(
                    cu_mask_sparse, mask_indptr, bitorder="little"
                )
                self.packed_custom_mask[layer_id] = cu_packed_mask
            self.cu_page_lengths = (
                torch.ones((num_seqs,), dtype=torch.int32, device="cuda") * seq_length
            )

        if self.mode == "LS_flatten_sum_prefill":
            for layer_id in range(num_layers):
                cu_mask_sum = (
                    torch.cat(masks_sum[layer_id], dim=0).contiguous().view(-1)
                )
                cu_packed_mask, _ = segment_packbits(
                    cu_mask_sum, mask_indptr_sum, bitorder="little"
                )
                self.packed_custom_mask[layer_id] = cu_packed_mask
            self.cu_page_lengths = (
                torch.ones((num_seqs,), dtype=torch.int32, device="cuda")
                * seq_length_sum_layers
            )
        if self.mode == "LS_rewrite_prefill":
            self.cu_page_lengths = (
                torch.ones((num_seqs,), dtype=torch.int32, device="cuda") * num_selected
            )

    def LA_generate_layouts(self, num_seqs, seq_length, num_selected):
        nums_selected = [
            int(seq_length * (1 / 2) * (0.890533**l)) for l in range(num_layers)
        ]
        ...
        mask_indptr_sparse = torch.cumsum(
            torch.tensor(mask_indptr_sparse, dtype=torch.int32, device="cuda"), dim=0
        )
        ...
        mask_indptr_max = torch.cumsum(
            torch.tensor(mask_indptr_max, dtype=torch.int32, device="cuda"), dim=0
        )
        ...
        for seq_id in range(num_seqs):
            ...
            if self.mode == "LA_flatten_max_prefill":
                self.global_block_table[seq_id, :seq_length_max_layers] = (
                    block_table_ids[:seq_length_max_layers].to(torch.int32)
                )
            ...
            for layer_id in range(num_layers):
                selected_indices_sparse = selected_indices_all_layers[
                    nums_selected_indptr[layer_id] : nums_selected_indptr[layer_id + 1]
                ]
                ...
                cu_mask_max = torch.zeros(
                    (seq_length_max_layers,), dtype=torch.bool, device="cuda"
                )
                ...
                if self.mode == "LA_flatten_max_prefill":
                    ...
                    self.dst_page_indptr_list.append(
                        self.global_block_table[seq_id, :seq_length_max_layers].to(
                            torch.int32
                        )
                    )
        ...
        if self.mode == "LA_flatten_max_prefill":
            for layer_id in range(num_layers):
                cu_mask_max = (
                    torch.cat(masks_max[layer_id], dim=0).contiguous().view(-1)
                )
                cu_packed_mask_max, _ = segment_packbits(
                    cu_mask_max, mask_indptr_max, bitorder="little"
                )
        ...
        if self.mode == "LA_sparse_prefill":
            for layer_id in range(num_layers):
                ...
                cu_packed_mask_sparse, _ = segment_packbits(
                    cu_mask_sparse, mask_indptr_sparse, bitorder="little"
                )
    ...
    def HS_generate_layouts(self, num_seqs, seq_length, num_selected):
        nums_selected = [
            int(seq_length * (1 / 2) * (0.502017**l)) for l in range(num_kv_heads)
        ]
        ...
        for seq_id in range(num_seqs):
            ...
            for head_id in range(num_kv_heads):
                ...
                block_table_ids_sum.append(
                    block_table_ids_per_head[:seq_length_sum_heads]
                    + head_id * num_blocks
                )
                block_table_ids_max.append(
                    block_table_ids_per_head[:seq_length_max_heads]
                    + head_id * num_blocks
                )
            ...
            if self.mode == "HS_sparse_prefill":
                self.global_block_table[
                    seq_id * num_kv_heads : (seq_id + 1) * num_kv_heads,
                    : block_table_ids.shape[-1],
                ] = block_table_ids.to(torch.int32)
            if self.mode == "HS_flatten_sum_prefill":
                self.global_block_table[
                    seq_id * num_kv_heads : (seq_id + 1) * num_kv_heads,
                    : block_table_ids_sum.shape[-1],
                ] = block_table_ids_sum.to(torch.int32)
            if self.mode == "HS_flatten_max_prefill":
                self.global_block_table[
                    seq_id * num_kv_heads : (seq_id + 1) * num_kv_heads,
                    : block_table_ids_max.shape[-1],
                ] = block_table_ids_max.to(torch.int32)
            ...
            for layer_id in range(num_layers):
                ...
                for head_id in range(num_kv_heads):
                    ...
                    selected_indices_sparse.append(
                        selected_indices_sparse_per_head + head_id * seq_length
                    )

                    mask_sparse_per_head = torch.zeros(
                        (seq_length,), dtype=torch.bool, device="cuda"
                    )
                    mask_sparse_per_head[selected_indices_sparse_per_head] = True
                    cu_mask_sparse[layer_id].append(mask_sparse_per_head)

                    mask_sum_per_head = torch.zeros(
                        seq_length_sum_heads, dtype=torch.bool, device="cuda"
                    )
                    mask_sum_per_head[
                        nums_selected_inptr[head_id] : nums_selected_inptr[head_id + 1]
                    ] = True
                    cu_mask_sum[layer_id].append(mask_sum_per_head)

                    mask_max_per_head = torch.zeros(
                        seq_length_max_heads, dtype=torch.bool, device="cuda"
                    )
                    ...
                    if self.mode == "HS_flatten_max_prefill":
                        self.src_page_indptr_list.append(
                            block_table_ids.view(-1)[selected_indices_sparse[-1]].to(
                                torch.int32
                            )
                        )
                        self.dst_page_indptr_list.append(
                            self.global_block_table[
                                seq_id,
                                seq_length * head_id : seq_length * head_id
                                + nums_selected[head_id],
                            ].to(torch.int32)
                        )

                selected_indices_sparse = (
                    torch.cat(selected_indices_sparse, dim=0).contiguous().view(-1)
                )

                if self.mode == "HS_flatten_sum_prefill":
                    self.src_page_indptr_list.append(
                        block_table_ids.view(-1)[selected_indices_sparse].to(
                            torch.int32
                        )
                    )
                    self.dst_page_indptr_list.append(
                        self.global_block_table[seq_id, :seq_length_sum_heads].to(
                            torch.int32
                        )
                    )

                cu_mask_sparse[layer_id] = (
                    torch.cat(cu_mask_sparse[layer_id], dim=0).contiguous().view(-1)
                )
                mask_sparse[layer_id].append(cu_mask_sparse[layer_id])

                cu_mask_sum[layer_id] = (
                    torch.cat(cu_mask_sum[layer_id], dim=0).contiguous().view(-1)
                )
                mask_sum[layer_id].append(cu_mask_sum[layer_id])

                cu_mask_max[layer_id] = (
                    torch.cat(cu_mask_max[layer_id], dim=0).contiguous().view(-1)
                )
        ...
        if self.mode == "HS_sparse_prefill":
            for layer_id in range(num_layers):
                cu_mask_sparse = (
                    torch.cat(mask_sparse[layer_id], dim=0).contiguous().view(-1)
                )
                cu_packed_mask_sparse, _ = segment_packbits(
                    cu_mask_sparse, mask_indptr_sparse, bitorder="little"
                )
                self.packed_custom_mask[layer_id] = cu_packed_mask_sparse
            self.cu_page_lengths = (
                torch.ones((num_seqs * num_kv_heads,), dtype=torch.int32, device="cuda")
                * seq_length
            )
        ...
        if self.mode == "HS_flatten_sum_prefill":
            for layer_id in range(num_layers):
                ...
                cu_packed_mask_sum, _ = segment_packbits(
                    cu_mask_sum, mask_indptr_sum, bitorder="little"
                )
    ...
    def HA_generate_layouts(self, num_seqs, seq_length, num_selected):
        ...
        for seq_id in range(num_seqs):
            ...
            for head_id in range(num_kv_heads):
                ...
                block_table_ids.append(block_table_ids_per_head + head_id * num_blocks)
                block_table_ids_max.append(
                    block_table_ids_per_head[:seq_length_max] + head_id * num_blocks
                )
            ...
            for layer_id in range(num_layers):
                ...
                for head_id in range(num_kv_heads):
                    selected_indices_sparse_per_head = torch.randint(
                        0,
                        seq_length,
                        (int(nums_selected[layer_id, head_id]),),
                        device="cuda",
                    )
                    ...
                    mask_mask_per_head = torch.zeros(
                        seq_length_max, dtype=torch.bool, device="cuda"
                    )
    ...
    def headwise_prepare(self):
        ...
        kv_indptr = torch.zeros(
            (self.cu_page_lengths.shape[0] + 1), device="cuda", dtype=torch.int32
        )
        kv_indptr[1:] = torch.cumsum(self.cu_page_lengths, dim=0)

        kv_page_indices = torch.empty(kv_indptr[-1], dtype=torch.int32, device="cuda")
        ...
        self.paged_prefill_wrapper.begin_forward(
            ...
            num_kv_heads=1,
            packed_custom_mask=(
                self.packed_custom_mask[0]
                if 0 in self.packed_custom_mask.keys()
                else None
            ),
    ...
    def sparse_prepare(self):
        ...
        self.block_sparse_wrapper.begin_forward(
            ...
            head_dim=head_dim,
            packed_mask=(
                self.packed_custom_mask if hasattr(self, "packed_custom_mask") else None
            ),
            q_data_type=torch.bfloat16,
    ...
    def flatten_sparse_compute(self, q, k_cache, v_cache):
        q = q.view(-1, num_kv_heads, num_heads // num_kv_heads, head_dim).reshape(
            -1, num_heads // num_kv_heads, head_dim
        )
        self.forward_wrapper.partial_update_custom_mask(
            self.packed_custom_mask[self.layer_id]
        )
        o = self.forward_wrapper.forward(q, (k_cache, v_cache))
        o = o.view(-1, num_kv_heads, num_heads // num_kv_heads, head_dim).view(
    ...
    def rewrite(self):
        # self.k_cache[self.dst_page_indptr_list] = self.k_cache[self.src_page_indptr_list]
        # self.v_cache[self.dst_page_indptr_list] = self.v_cache[self.src_page_indptr_list]
        inplace_rewrite_flattened(
            self.k_cache,
            self.v_cache,
            self.src_page_indptr_list,
            self.dst_page_indptr_list,
...
if __name__ == "__main__":
    ...
    HA_flatten_max_stats = bench.run(
        "HA_flatten_max_prefill", num_blocks, num_seqs, seq_lengths
    )
